            ...
            <div class="row clearfix">
                <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                    <div class="card">
                        <div class="header">
                            <h2>
                                ADD MAHASISWA KELAS <?php echo $kode_kelas ?>
                            </h2>
                        </div>
                        <div class="body">
                            <div class="row clearfix">
...
<?php
    ...
    if (isset($_POST['submit'])) {
        ...
        //cek dia udah ada apa belom
        $sql = "SELECT * FROM detail_kelas WHERE kode_kelas='$kode_kelas' AND nim='$nim'";
        $query = mysqli_query($connect, $sql);
        if (mysqli_num_rows($query) >= 1) {
            echo '<script>alert("Dia sudah terdaftar pada kelas ini. Data Gagal disimpan!");window.location.href=\'../kelas\';</script>';
        } else {
            //cek udah penuh apa belom
            $sql = "SELECT * FROM detail_kelas WHERE kode_kelas='$kode_kelas'";
            $query = mysqli_query($connect, $sql);
            if (mysqli_num_rows($query) >= 5) {
                echo '<script>alert("Kelas sudah penuh. Data Gagal disimpan!");window.location.href=\'../kelas\';</script>';
            } else {
                $sql = "INSERT INTO detail_kelas(kode_kelas, nim) VALUES ('$kode_kelas', '$nim')";
                $query = mysqli_query($connect,$sql);
                if ($query) {
